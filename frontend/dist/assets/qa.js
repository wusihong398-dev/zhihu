(() => {
  ...
  async function authenticatedBlob(path) {
    const token = sessionStorage.getItem("totod_token") || "";
    const response = await fetch(`/api${path}`, { credentials: "same-origin", headers: token ? { Authorization: `Bearer ${token}` } : {} });
  ...
  function renderAnswers() {
    ...
    $$(".answer-diagnostic", list).forEach((button) => button.addEventListener("click", () => openFailureScreenshot(button.closest("[data-answer-id]").dataset.answerId)));
    updateAnswerSelection();
  }
  async function openFailureScreenshot(answerId) {
    const accountId = $("#answer-list-account").value;
    const popup = window.open("", "_blank");
    try {
      const blob = await authenticatedBlob(`/accounts/${accountId}/answers/${answerId}/failure-screenshot`);
  ...
  function renderJob(job, prefix) { const empty = !job; $(`#${prefix}-status`).textContent = empty ? "暂无任务" : ({ pending: "等待中", running: "运行中", paused: "已暂停", stopped: "已停止", completed: "已完成", failed: "任务失败" })[job.status]; $(`#${prefix}-percent`).textContent = `${empty ? 0 : (job.progress_percent ?? 0)}%`; $(`#${prefix}-count`).textContent = empty ? "0 / 0" : `${job.completed_count ?? 0} / ${job.total_count ?? 0} · 成功 ${job.success_count ?? 0} · 失败 ${job.failed_count ?? 0}`; $(`#${prefix}-bar`).style.width = `${empty ? 0 : (job.progress_percent ?? 0)}%`; $(`#${prefix}-current`).textContent = empty ? "暂无任务" : job.current_item || "全部处理完成"; $(`#${prefix}-message`).textContent = empty ? "" : job.error_message || ""; const active = job && !terminal.has(job.status); $(`#${prefix}-pause`).disabled = !active || job.status !== "running"; $(`#${prefix}-resume`).disabled = !active || !["paused", "pending"].includes(job.status); $(`#${prefix}-stop`).disabled = !active; }
  async function pollJobs() {
    ...
    const tick = async () => {
      try {
        ...
        if (state.publishJob && !terminal.has(state.publishJob.status)) { state.publishJob = await api(`/answer-jobs/${state.publishJob.id}`); renderJob(state.publishJob, "answer-job"); renderJob(state.publishJob, "auto-job"); if (terminal.has(state.publishJob.status)) { const finishedJob = state.publishJob; await loadAnswers(true); await loadAutoSummary(false); toast((finishedJob?.failed_count ?? 0) ? "发布任务完成，失败原因和诊断图已保存" : "回答发布完成", (finishedJob?.failed_count ?? 0) ? "error" : "success"); } }
      } catch (error) { toast(error.message, "error"); }
  ...
  async function loadAutoSummary(refreshJob = true) { const accountId = $("#auto-answer-account").value; $("#auto-answer-run").disabled = !accountId; if (!accountId) return; try { const data = await api(`/accounts/${accountId}/auto-answer/summary`); $("#auto-daily-limit").textContent = data.daily_limit; $("#auto-published").textContent = data.published_today; $("#auto-remaining").textContent = data.remaining_today; $("#auto-ready").textContent = data.ready_count; $("#auto-answer-run").disabled = !data.remaining_today || !data.ready_count; if (refreshJob !== false) await loadLatestPublish(accountId); } catch (error) { toast(error.message, "error"); } }
