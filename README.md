...
`v0.8.1`：文章生成与文章列表已接入。用户可按知乎账号选择关键词文件夹、批量勾选关键词、绑定推广商品及已配置的 AI 模型，设置每个关键词篇数、字数范围和提示词后生成草稿。文章列表支持全部账号汇总、搜索、状态筛选、每页 100 篇、编辑、单删、批量删除及批量转为待发布；修复从文章生成页进入 AI 配置后平台卡片不显示的问题。所有文章与 AI 配置继续按系统用户和知乎账号隔离。
