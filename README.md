...
`v0.19.5`：修复 Edge 内嵌客户端向知乎文章插入超大本地图片时出现“当前素材文件过大”并卡住发布的问题。发布前会将图片等比例缩放到最长边 1024 像素后再粘贴；若知乎仍拒绝图片或图片处理失败，客户端会记录原因、跳过图片并继续发布正文，避免整篇文章任务阻塞。

Windows 构建包只包含 `TOTOD-Edge-Publisher-v0.19.5.exe`。

## v0.19.4

`v0.19.4`：修复新账号文章发布任务一直停在“等待 Windows 本地发布客户端领取”的问题。Edge 内嵌客户端现在会记住每个已登录账号并自动轮询多个账号的回答和文章任务；新增账号后可直接刷新账号列表，账号登录一次并加入监听后，后续无需手动切换账号领取任务。客户端重启后会自动恢复已记住账号的监听；若某个账号登录失效，会明确提示重新登录，不影响其他账号继续监听。

Windows 构建包现在只包含 `TOTOD-Edge-Publisher-v0.19.4.exe`。旧版 `TOTOD-Windows-Publisher.exe` 使用外部 Chrome 且不支持文章任务，已从下载包移除，避免误启动后任务一直等待领取。

## v0.19.3

`v0.19.3`：修复新增或编辑知乎账号时无法选择配置来源账号的问题。系统管理员现在可从任意其他知乎账号复制运营配置，普通用户仍只能同步本人账号；来源账号下拉框始终显示，并明确提示当前是否有可同步账号，不再出现复选框不可用却没有原因的情况。登录资料、Cookie、文章、问题、回答和发布历史仍保持独立。

## v0.19.2

`v0.19.2`：修复“生成失败”空文章被再次加入发布队列后，Windows 客户端报 `Value cannot be null (Parameter 'text')` 的问题。服务端现在会在创建发布任务前拦截空标题或空正文，保留生成失败记录并明确提示重新生成或编辑；后台禁用空正文文章的单篇与批量发布按钮；客户端增加空正文兜底，不再调用系统剪贴板导致空值错误。

## v0.19.1

`v0.19.1`：修复知乎新版文章创作页正文编辑器无法识别的问题。客户端会先定位并激活“请输入正文”区域，再通过编辑器事件或系统粘贴写入正文并校验实际字数；同时扩充 ProseMirror、DraftEditor 等选择器和结构诊断。若知乎再次调整编辑器结构，客户端会在首篇失败后停止监听，保留其余待发布任务，避免整批文章连续失败。

## v0.19.0

`v0.19.0`：Windows Edge 内嵌发布客户端新增知乎文章发布能力。账号选择“Windows 本地客户端”后，批量文章发布不再使用服务器 Chromium，而是进入本地任务队列，在账号独立的内置 Edge 会话中自动填写标题、正文、本地图片并完成正式发布；公开文章链接、发布时间和逐篇失败原因自动回传后台。新版客户端同时领取文章与回答任务，旧版客户端仍只领取回答任务，避免版本混用误发。

## v0.18.7

- 修复火山方舟部分新模型只支持默认 temperature、而文章生成固定传入 0.8 导致全部返回 HTTP 400 的问题。
- 文章生成与回答生成不再强制传入 temperature，由各 AI 模型使用自己的兼容默认值；“测试连接成功但正式生成失败”的差异已消除。

## v0.18.6

- 文章生成任务会在进度区逐篇列出“关键词＋失败原因”，完成后不再只显示“请到文章列表查看”的笼统提示。
- 文章生成进度新增“查看失败文章”按钮，自动切换到当前知乎账号的“文章 → 发布失败”栏目，直接查看每篇文章的完整失败原因。
- 草稿回答列表表头新增始终可见的“全选当前页”，与原批量工具栏同步，勾选后可批量发布、转为待发布或删除。

## v0.18.5

- 修复文章生成调用可能长时间不结束的问题：除网络读写超时外增加单篇 150 秒整次调用硬超时，到时自动终止并记录明确失败原因，不再无限等待。
- 文章生成进度会显示当前使用的 AI 平台、关键词、单篇最长等待时间和实时已等待秒数，便于区分模型仍在生成与任务异常。
- 修复文章任务进度轮询遇到一次临时网络错误后永久停止的问题；页面会显示断线原因并每 3 秒自动重试，恢复后继续刷新进度，无需手动重开页面。

## v0.18.4

- 修复本地 Windows 客户端发布失败但服务器没有诊断截图时，点击“诊断图”只打开空白窗口后立即闪退的问题；现在诊断窗口会保留并明确说明无截图原因，同时提示查看列表失败原因和客户端日志。
- 问题列表新增“采集时间”，直接显示每个问题真实保存的发现时间；历史问题无需迁移即可显示。
- 回答列表将时间列改为“回答时间 / 尝试时间”：已发布回答明确显示真实回答成功时间，失败回答显示最近尝试时间，避免混淆。

## v0.18.3

- 修复知乎新版图文回答编辑器把右下角“发布回答”按钮放在编辑器表单容器外部，导致 v0.18.2 已填好正文却无法点击最终发布的问题。
- 客户端现在优先在整个可见页面精确查找“发布回答/提交回答”，只有“发布”短文案仍限制在当前回答表单内，避免误点评论发布按钮；若结构再次变化，失败日志会列出当前识别到的按钮文字用于诊断。

## v0.18.2

- 修复 WebView2 客户端在知乎问题页误把无关的通用编辑区域识别为回答编辑器，导致跳过顶部“写回答”按钮并报“未找到可用的发布回答按钮”的问题。
- 发布流程改为只识别可见的“写回答”入口和回答表单内的编辑器、发布按钮；点击入口后持续等待编辑器加载，并在日志中显示“点击写回答、填写内容、点击发布回答”三个阶段。
- 新增知乎回答提交接口响应监听；即使发布后页面没有自动跳转，也能从成功响应中取得真实回答编号并回写回答链接。
- 修复同一客户端切换知乎账号或重新登录时重复使用已释放 WebView2 控件的问题，各账号继续使用相互隔离的登录资料。
...
- 检测到 `40362` 后立即停止监听并回写真实失败原因，避免继续领取和连续访问加重账号限制。

## v0.17.4

- 修复 Windows 客户端只打开知乎登录窗口、但用户还必须再次点击“开始监听发布任务”，导致后台任务一直停在 `0%` 的易错流程。
- 点击“登录并自动监听”后，客户端检测到知乎登录 Cookie 会自动领取任务；监听中按钮、停止按钮、状态和日志会同步显示真实运行状态。

## v0.17.3

- 修复 Windows 客户端界面已刷新出账号、但发布工作线程没有保存账号列表，导致点击“开始监听发布任务”后无法领取任务、后台一直停在 `0%` 的问题。
- 开始监听前会刷新并校验账号，客户端标题显示版本号，状态和运行日志会明确显示正在监听的账号；刷新账号时保留当前选择。
- Windows 构建流程新增监听线程回归测试，防止再次出现界面有账号但工作线程无法启动的问题。

## v0.17.2

- 修复 Windows 客户端只查找用户目录 Edge、导致系统目录已安装 Edge 仍提示 `msedge is not found` 的问题。
- 客户端会依次检测系统 Edge、用户 Edge、系统或用户 Chrome，并显示实际启动的浏览器路径。

## v0.17.1

- 修复 Windows 本地发布客户端连接成功但刷新不到知乎账号的问题：账号列表不再隐藏尚未切换发布方式的账号，并支持管理员设备查看和领取有权限的账号任务。
- 客户端会标出“本地发布”或“需切换为本地发布”，未切换时给出明确操作提示。

## v0.17.0

- 新增 Windows 本地发布客户端和设备密钥管理。回答任务可按知乎账号选择由本机可见 Edge 发布，使用用户电脑的常用网络，避免服务器 IP 触发知乎 40362。
- 本地客户端按账号隔离 Edge 资料目录，发布成功必须回传真实知乎回答地址；失败原因、尝试时间和批次进度继续回写后台，失败不占每日回答额度。
- “登录官网”明确标注为服务器登录官网，账号编辑新增回答发布方式选择。
...
`v0.15.3`：修复服务器前后端版本短暂不一致时，回答提示词模板接口的 404 会连带中断账号和 AI 配置加载的问题。问答模块现在先独立加载账号与 AI 平台，模板接口异常只禁用模板操作并给出明确升级提示，不再清空账号下拉框；部署脚本同时补齐回答模板样式文件。

`v0.16.0`：将“商品管理”和“提示词模板”提升为独立一级栏目。商品仍按知乎账号隔离，文章与回答生成页会按当前账号重新加载启用商品并明确显示所属账号；提示词模板页集中管理文章标题、文章正文和回答模板，两类模板共用当前系统用户自己的文件夹，支持新建、保存、改名、搜索和删除。

`v0.16.1`：文章提示词模板与问答提示词模板改为完全独立保存和设置，各自拥有独立文件夹、模板列表及增删改操作。升级时会自动复制并迁移 v0.16.0 的问答模板文件夹关联，保留已有模板和分类。

`v0.16.2`：回答每日额度改为只统计知乎确认发布成功的回答。发布失败仍保留尝试时间和失败原因，但不再占用额度，可修正问题后继续重试。

`v0.16.6`：识别知乎风险控制错误 40362，并显示准确恢复建议；一个账号触发风控后立即停止本批回答发布，不再逐条连续请求，避免加重限制。失败回答不计入每日成功发布额度，可稍后重试或人工发布。

`v0.16.5`：修复回答任务完成后右上角读取 `failed_count` 空值报错，避免自动回答摘要覆盖当前发布任务；发布失败记录新增鉴权保护的“诊断图”，可直接查看服务器发布时实际打开的知乎页面；补充知乎“回答”入口兼容并延长动态加载等待。

`v0.16.4`：修复知乎新版问题页无法识别“写回答”入口的问题。回答发布兼容带图标、嵌套文字及新版文案的按钮与编辑器，增加延迟加载等待和遮挡点击兜底，并分别提示问题已关闭、已删除、账号已回答或回答权限受限。

`v0.16.3`：新增同一系统用户下的知乎账号配置同步。新增或编辑账号时可选择另一账号作为来源，一次同步每日额度、时区、关键词循环规则、推广商品、关键词与文件夹及定时计划；同名配置合并更新，目标账号其他配置保留。知乎登录 Profile、Cookie、文章、问题、回答和发布历史始终独立，用户级 AI API、提示词模板与本地图片继续自动共用。
