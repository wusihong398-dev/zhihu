...
def test_health_version_and_auth_boundary() -> None:
    with TestClient(app) as client:
        ...
        assert version.json()["version"] == "0.19.5"
...
def test_webview2_article_editor_supports_new_zhihu_structure() -> None:
    ...
    assert 'const string AppVersion = "0.19.5"' in publisher
    ...
    assert "ArticleImageMaxDimension = 1024" in publisher
    assert "已跳过图片并继续发布正文" in publisher
    assert "<Version>0.19.5</Version>" in project
    assert "<AssemblyName>TOTOD-Edge-Publisher-v0.19.5</AssemblyName>" in project
...
def test_article_list_requires_one_account_at_a_time() -> None:
    ...
    assert "app.js?v=0.19.5" in index
